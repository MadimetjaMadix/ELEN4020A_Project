...
int main( int argc, char *argv[] )
{
    ...
    comm = MPI_COMM_WORLD;
    err = MPI_File_open( comm, "data.bin", MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh );
    if (err)
    {
    ...
    err = MPI_File_open( comm, "data.bin", MPI_MODE_RDONLY | MPI_MODE_SEQUENTIAL | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &fh );
    if (err)
    {
        MPI_Abort(MPI_COMM_WORLD, 911);
    }
